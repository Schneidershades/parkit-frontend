		...
		<q-tabs
          ...
          v-if="useFreeWash=='yes' && freeWash == true"
			...
			<div class="cbs-vehicle-list">
				<q-tab @click="dialog = true" class="col-md-3 vehicle-cbs"  v-for="product in products.data" :name="product.slug" :label="product.type" v-if="product.type== checkPlateNumber.vehicle_type"
						...
						<q-tab-panel :name="product.slug" v-for="product in products.data" :key="product.id" >
							...
							<div class="lt-md">
								<q-list class="rounded-borders">
									<q-item class=" col-md-12" v-for="content in product.packages" v-if="content.venue=='parkit-location' && content.package == 'BASIC PACKAGE (Basic wash)'" :key ="content.slug">
