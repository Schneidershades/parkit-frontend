<template>
	<q-card flat>
		...
		<q-tabs
          ...
          color="primary"
          class=""
          v-if="useFreeWash=='yes' && freeWash == true"
			>
			<div class="cbs-vehicle-list">
				<template v-for="product in products.data" >
					<q-tab @click="dialog = true" 
						...
						:name="product.slug" 
						:label="product.type" 
						v-if="product.type== checkPlateNumber.vehicle_type" 
		          	...
		          	<q-tab-panels bordered v-model="tab" animated class="row" id="items" >
						<q-tab-panel :name="product.slug" v-for="product in products.data" :key="product.id" >
							...
							<div class="lt-md">
								<q-list class="rounded-borders">
									<template  v-for="content in product.packages" >
										<q-item class=" col-md-12" v-if="content.venue=='parkit-location' && content.package == 'BASIC PACKAGE (Basic wash)'" :key ="content.slug">
							...
							<div class="row wrap tab-card gt-sm">
								<template v-for="content in product.packages" >
									<div class="q-pa-sm col-md-3" v-if="content.venue=='parkit-location' && content.package == 'BASIC PACKAGE (Basic wash)'" :key="content.id">
		          	...
		          	<q-tab-panels bordered v-model="tab" animated class="row" id="items" >
						<q-tab-panel :name="product.slug" v-for="product in products.data" :key="product.id">
							...
							<div class="lt-md">
								<q-list class="rounded-borders">
									<template v-for="content in product.packages" >
										<q-item class=" col-md-12" v-if="content.venue=='parkit-location'" :key ="content.slug">
											<q-item-section avatar top class="col-3 ">
												<q-icon name="account_tree" color="black" size="20px" />
												{{content.short_name}}
											</q-item-section>
							...
							<div class="row wrap tab-card gt-sm">
								<template v-for="content in product.packages">
									<div class="q-pa-sm col-md-3"  v-if="content.venue=='parkit-location'" :key="content.id">
										<q-card class="my-card" >
											<q-card-section class="bg-primary text-white card-style" >
												<div class="text-h6">{{content.package}}</div>
												<q-card-actions align="right" >
...
export default {
	...
	computed: {
		...mapGetters({
			products: 'webAdminShopping/products',
			tabsProducts: 'webAdminShopping/products',
	...
	methods:{
		...mapActions({
			getProducts: 'webAdminShopping/getProducts',
			addProductToCart: 'webAdminShopping/addProductToCart',
